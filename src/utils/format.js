 ...
 */
export const saleMoneyFormat = ({ value, returnType }) => {
	// 中文
	if (language === zhCN) {
 ...
 * returnType: join返回字符串 split返回对象{int:''整数部分, point:''小数点, float:''小数部分, unit:''单位}
 */
export const passengerNumFormat = ({ value, returnType }) => {
	// 中文
	if (language === zhCN) {
 ...
 */
export const frequencyFormat = ({ value, returnType }) => {
	// 中文
	if (language === zhCN) {
		return {
 ...
 * @returns {string}
 */
export const separateWhenEn = (value) => {
	// 中文
	if (language === zhCN) {
		return value;
	}
 ...
 */
export const formatFloatByPermile = ({ value, returnType = RETURNModel.join }) => {
	let result = value;
 ...
 */
export const formatFloatByPercent = ({ value, returnType = RETURNModel.join }) => {
	let result = value;
	if (result === 0) {
		result = '0%';
