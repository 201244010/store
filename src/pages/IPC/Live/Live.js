...
@connect(
	state => {
		const {
			faceid: { rectangles, list },
			live: { ppi, streamId, ppiChanged, timeSlots },
		} = state;
								...
								renderItem={item => (
									<List.Item key={item.id}>
										...
										>
											...
											<p>{`(${genders[item.gender]} ${item.age}岁)`}</p>
											<p>
												<span>
													{formatMessage({
											...
											<p>
												<Link
													className={styles['button-infos']}
													to="./userinfo"
												>
													{formatMessage({ id: 'live.enter.details' })}
												</Link>
											</p>
										</Card>
										{/* <Card
