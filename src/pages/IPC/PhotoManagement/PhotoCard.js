...
class PhotoCard extends React.Component {
		...
		form.validateFields(async errors => {
			if (!errors) {
				...
				if (
					...
					file.response.data.verifyResult === 1
				) {
	...
	handleInfo = type => {
		...
		switch (type) {
			case 'name':
				return name === '' ? formatMessage({ id: 'photoManagement.unKnown' }) : name;
			case 'gender':
				if (gender === 1) {
			...
			case 'latestDate':
				return latestDate === 0
					? formatMessage({ id: 'photoManagement.unKnown' })
					: this.dateStrFormat(latestDate);
			case 'createDate':
				return createDate === 0
					? formatMessage({ id: 'photoManagement.unKnown' })
					: this.dateStrFormat(createDate);
			case 'count':
			...
			>
				<div>
					...
					<div className={styles['word-col']}>
						...
						<span className={styles['info-card-span']}>
							<span>
								{formatMessage({ id: 'photoManagement.card.latestTime' })} :{' '}
							</span>
						...
						<Row className={styles['edit-content-span']}>
							<Col span={10}>
								...
								>
									<div className={styles['upload-pic']}>
										...
										>
											...
											<p>
												{formatMessage({
													id: 'photoManagement.card.changeImg',
												})}
											</p>
										</div>
									</div>
								...
								{uploadFail && (
									<div>
										<p>
											{formatMessage({
												id: 'photoManagement.card.uploadFail',
											})}
										</p>
										<Button
											onClick={() =>
												this.setState({ file: {}, uploadFail: false })
											}
										>
								...
								{uploadSuccess && (
									<div>
										<p>
											{formatMessage({
												id: 'photoManagement.card.uploadSuccess',
											})}
										</p>
										<Button
											onClick={() =>
							...
							<Col span={14}>
								<div className={styles['edit-form']}>
									...
									>
										<Form.Item
											label={formatMessage({ id: 'photoManagement.name' })}
										>
												...
												rules: [
													{
														...
														message: formatMessage({
															id: 'photoManagement.card.alert1',
														}),
													},
													{
														max: 20,
														message: formatMessage({
															id: 'photoManagement.card.alert2',
														}),
													},
												],
												initialValue: name,
										...
										<Form.Item
											label={formatMessage({ id: 'photoManagement.gender' })}
										>
											{getFieldDecorator('gender', {
												...
												rules: [
													{
														...
														message: formatMessage({
															id:
																'photoManagement.card.genderMessage',
														}),
													},
												],
											})(
												<Select>
													<Option value={1} key={1}>
														{formatMessage({
															id: 'photoManagement.male',
														})}
													</Option>
													<Option value={2} key={2}>
														{formatMessage({
															id: 'photoManagement.female',
														})}
													</Option>
												</Select>
											)}
										...
										<Form.Item
											label={formatMessage({
												id: 'photoManagement.card.libraryName',
											})}
										>
										...
										<Form.Item
											label={formatMessage({ id: 'photoManagement.age' })}
										>
												...
												rules: [
													{
														required: true,
														message: formatMessage({
															id: 'photoManagement.card.ageMessage',
														}),
													},
												],
											})(
										...
										<Form.Item
											label={formatMessage({
												id: 'photoManagement.card.frequency',
											})}
										>
										...
										<Form.Item
											label={formatMessage({
												id: 'photoManagement.card.createTime',
											})}
										>
										...
										<Form.Item
											label={formatMessage({
												id: 'photoManagement.card.latestTime',
											})}
										>
											<span>{this.handleInfo('latestDate')}</span>
										</Form.Item>
					...
					<Row className={styles['move-modal']}>
						...
						<Col span={16}>
							...
							<Row className={styles['content-radio']}>
								<Col span={4} className={styles.move}>
									{' '}
									{formatMessage({ id: 'photoManagement.moveTo' })}
								</Col>
