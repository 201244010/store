...
class PhotoManagement extends React.Component {
	...
	componentDidMount() {
		const { getLibrary, getRange, location: { query: { groupId }}} = this.props;
    ...
    // 全选
    handleFullChoose = e => {
    	const { check, photoLibrary: {photoList} } = this.props;
    ...
    // 判断本页是否全选
	isFullChecked = () => {
		const { photoLibrary: {checkList, photoList}} = this.props;
	...
	reset = () => {
		const { clear, getPhotoList, form: { resetFields} } = this.props;
		const { groupId } = this.state;
		const header = {
			pageNum: 1,
			pageSize: 9,
	...
	render() {
		...
		return(
			<Card className={styles['management-card']}>
						...
						<Row gutter={SEARCH_FORM_GUTTER.NORMAL}>
								...
								<Form.Item label={formatMessage({id:'photoManagement.gender'})}>
									{getFieldDecorator('gender', {
										...
										<Select onChange={value => this.handleGender(value)}>
											<Option value={-1}>{formatMessage({id:'photoManagement.all'})}</Option>
											<Option value={1}>{formatMessage({id:'photoManagement.male'})}</Option>
											<Option value={2}>{formatMessage({id:'photoManagement.female'})}</Option>
										</Select>,
									)}
								</Form.Item>
							</Col>
								...
								<Form.Item label={formatMessage({id:'photoManagement.age'})}>
									{getFieldDecorator('age', {
										...
										<Select onChange={value => this.handleAge(value)}>
											<Option value={10}>{formatMessage({id: 'photoManagement.all'})}</Option>
											{ageRange.map(item => <Option value={item.ageCode} key={item.ageCode}>{item.ageRange}</Option>)}
										</Select>,
									)}
								</Form.Item>
							</Col>
							<Col {...SEARCH_FORM_COL.ONE_THIRD}>
								<Form.Item label={formatMessage({id: 'photoManagement.keyword'})}>
									{getFieldDecorator('key', {
										...
										<Input
											placeholder={formatMessage({id: 'photoManagement.queryPlaceholder'})}
							...
							<Col {...SEARCH_FORM_COL.OFFSET_TWO_THIRD}>
								<Form.Item className={global['query-item']}>
									<Button onClick={this.onSearch} type='primary' className={styles['reset-btn']}>
										{formatMessage({id: 'photoManagement.buttonSearch'})}
									</Button>

									<Button onClick={this.reset} className={global['btn-margin-left']}>
				...
				<Button type="primary" onClick={this.showUpload} className={styles['add-photo-btn']}>
				...
				<Alert
					message={
						<div>
							<Checkbox
								checked={fullChosen}
								onChange={this.handleFullChoose}
							>
								{formatMessage({id: 'photoManagement.allChoose'})}
							</Checkbox>
							{formatMessage({id: 'photoManagement.alreadyChoose'})}{checkList.length}{formatMessage({id: 'photoManagement.type'})}
							{checkList.length > 0 && <a href="javascript:void(0);" onClick={clear}>{formatMessage({id: 'photoManagement.clear'})}</a>}
						</div>}
					type="info"
					showIcon={false}
				...
				>
					<Row>
						...
						<Col span={18}>
							<Radio.Group defaultValue={null} value={removeRadioValue} onChange={this.removeRadioChange}>
				...
				<Modal
					...
					width={528}
				>
					<Alert message={formatMessage({id:'photoManagement.addFailContent'})} type='error' />
					{failedList.map(
						(item, index) =>
							<div className={styles['fail-list-modal']} style={{backgroundImage: `url(${item.imgUrl})`}} key={index} />
					)}
				</Modal>
			</Card>
		);
