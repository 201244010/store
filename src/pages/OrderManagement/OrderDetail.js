...
const statusTitle = [formatMessage({id: 'orderManagement.title.waiting'}), '', '',
...
const serviceKinds = [
	formatMessage({id: 'orderManagement.detail.7daysCloudStorage'}),
	formatMessage({id: 'orderManagement.detail.30daysCloudStorage'}),
];

const iconUrl = [
	require('../../assets/icon/clock-circle.png'), '', '', require('../../assets/icon/success.svg'), require('../../assets/icon/close-dark.png')
];
...
class OrderDetail extends React.Component {
		...
		this.columns = [
			{
				...
				key: 'serviceType',
				render: value => serviceKinds[value - 1]
			}, {
				title: formatMessage({id: 'orderManagement.detail.imgUrl'}),
				dataIndex: 'imgUrl',
				key: 'imgUrl',
				render: value => <img className={styles['hold-pic']} src={(value==='' || value === undefined) ? hold : value} />
			}, {
				...
				key: 'expireTime',
				render: value => moment.unix(value).format('YYYY年MM月DD日')
			}, {
	...
	async componentDidMount() {
		const { location: { query: { orderNo }}, getOrderDetail,} = this.props;
	...
	confirmCancelModal = async () => {
		this.setState({showModal: false});
		const { cancel, getOrderDetail, location: { query: { orderNo }} } = this.props;
		await cancel({orderNo});
		await getOrderDetail({ orderNo });
	};
	
	pay = () => {
		const { location: { query: { orderNo }}, goToPath,} = this.props;
		...
		return (
			<Spin spinning={loading.effects['orderManagement/getOrderDetail']}>
				...
				<Card className={styles.content}>
					<ul>
						<li>
							<span className={styles.title}>{formatMessage({id: 'orderManagement.detail.orderInfo'})}</span>
							<Row>
								<Col span={8}>{formatMessage({id: 'orderManagement.detail.orderNo'})}{orderNo}</Col>
								<Col span={8}>{formatMessage({id: 'orderManagement.detail.createdTime'})}{moment.unix(createdTime).format('YYYY-MM-DD HH:mm:ss')}</Col>
								{
									status === 4 &&
									<Col span={8}>
										{formatMessage({id: 'orderManagement.detail.payType'})}
										{purchaseType === 'purchase-type-alipay' ? formatMessage({id: 'orderManagement.detail.aliPay'}) : '--'}
									</Col>
								}
								<Col span={8}>{formatMessage({id: 'orderManagement.detail.createdPerson'})}{issuerAccount}</Col>
								{
									status === 4 &&
										<Col span={8}>{formatMessage({id: 'orderManagement.detail.payPerson'})}{(payerAccount === undefined || payerAccount === '') ? issuerAccount : payerAccount}</Col>
								}
						...
						
						{
							invoiceInfo !== null &&
								<li>
									<span className={styles.title}>{formatMessage({id: 'orderManagement.detail.invoiceInfo'})}</span>
									<Row>
										<Col span={8}>
											{formatMessage({id: 'orderManagement.detail.invoiceType'})}
											{invoiceKinds[invoiceInfo.invoiceKind]}({invoiceTitleType[invoiceInfo.titleType - 1]})
										</Col>
										<Col span={8}>
											{formatMessage({id: 'orderManagement.detail.invoiceHead'})}
											{invoiceInfo.titleType === 1 ?
												formatMessage({id: 'orderManagement.detail.personal'})
												:
										...
										<Col span={8}>
											{formatMessage({id: 'orderManagement.detail.taxNo'})}{invoiceInfo.taxRegisterNo}
										</Col>}
									</Row>
									<Divider className={styles.divide} />
								</li>
						...
						<div className={styles['bottom-bar']}>
							{
								...
								<>
									<span>
										...
										<span className={styles['remain-time']}>{this.countDownFormat(remainTime)}</span>
									</span>
									<Button type='primary' className={styles['pay-at-once']} onClick={this.pay}>
										{formatMessage({id: 'orderManagement.operation.payImmediately'})}
									</Button>
									<Button className={styles['cancel-pay']} onClick={this.cancelOrderModal}>
							...
							{status === 5 &&
								<Button type='primary' className={styles['cancel-pay']} onClick={this.bugAgain}>
