...
class VideoPlayerProgressBar extends React.Component {
    ...
    setPosition(timestamp) {
        ...
        if (this.wrapper) {
            ...
            const x = Math.ceil(
                ((timeStart - current) * this.oneHourWidth) / (60 * 60) +
            ...
            <div className={styles['timebar-container']}>
                        ...
                        <Draggable
                            ...
                            bounds={(() => {
                                if (this.wrapper) {
                                    const left =
                                        Math.ceil(
                                            ((timeStart - moment().unix()) * oneHourWidth) /
                                                (60 * 60)
                                        ) +
                                        this.wrapper.offsetWidth / 2;
                            ...
                            onStart={(e, dragger) => {
                                ...
                                const time =
                                    ((Math.abs(dragger.x) + this.wrapper.offsetWidth / 2) /
                                        oneHourWidth) *
                                        60 *
                                        60 +
                                    timeStart;
                            ...
                            onDrag={(e, dragger) => {
                                ...
                                if (Math.abs(deltaX) > 0) {
                                    const time =
                                        ((Math.abs(x) + this.wrapper.offsetWidth / 2) /
                                            oneHourWidth) *
                                            60 *
                                            60 +
                                        timeStart;
                        ...
                        <>
                            <span
                                className={`${styles.icon} ${
                                    direction === 'left' ? styles.backward : ''
                                } }`}
                            />
                ...
                >
                    <div className={styles.pointer}>
                        <div
                            className={styles['pointer-wrapper']}
                            ref={wrapper => {
                                this.pointerWrapper = wrapper;
                            }}
                        />
                    </div>
                </Tooltip>
